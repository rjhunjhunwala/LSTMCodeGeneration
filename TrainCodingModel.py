...
with open("code.txt","r") as fil:
    s = fil.read()
for i in range(0, 16000):#len(s)-INPUT_SEQUENCE_LENGTH-1):
...
y = np.array(y)
model = Sequential()
model.add(LSTM(128, return_sequences = True, input_shape = (INPUT_SEQUENCE_LENGTH, VOCAB_SIZE)))
model.add(LSTM(128))
model.add(Dense(VOCAB_SIZE, activation='softmax'))
print(model.summary())
model.compile(loss='categorical_crossentropy',optimizer='adam')
model.fit(X ,y,epochs = 35, verbose = 2)
model.save("codingModel.h5")